...
generate() async {
  String appDocumentsPath = "lib";

  const namePackage = "bakery_manager";

  // String schemaContent = await rootBundle.loadString('lib/schema.graphql');
  String schemaContent =
      await File('lib/graphql/schema.graphql').readAsString();

  schemaContent = schemaContent.replaceAll("ID", 'String');

  // final schemaFile = File('schema.graphql');
  ...
  final schema = gql_schema.GraphQLSchema.fromNode(
    lang.parseString(
      // schemaContents,
      schemaContent,
      url: "lib/graphql/schema.graphql",
    ),
  );
  ...
  models.forEach((className, modelContent) async {
    final modelFile = File(
        '${outputDirectory.path}/generated/${formatNameTypeForImport(className)}.dart');
